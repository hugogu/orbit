...
- The install cache contains the explorer HTML, all compiled scripts/styles (including astronomy workers), PWA assets and basic planet textures, about 9 MiB. The explorer uses that build's saved HTML so its chunks stay in sync. URL language, selection and observer parameters are still interpreted by the client.
