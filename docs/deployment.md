...
- The install cache contains the explorer HTML, all compiled scripts/styles (including astronomy workers), PWA assets, and the Earth, Sun, and Milky Way textures; the build reports 4.6 MiB plus the explorer HTML. The explorer uses that build's saved HTML so its chunks stay in sync. URL language, selection and observer parameters are still interpreted by the client.
...
- Additional textures, terrain, models and profile images load on demand. Their runtime cache is capped at 80 entries for 30 days and is removed on a new build. Quota pressure can evict runtime caches; clearing browser storage removes all offline content. High-resolution textures retain the existing standard-quality fallback.
