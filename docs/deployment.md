...
- Moon and Mars textures preload when the browser is idle; other textures, terrain, models and profile images load when needed. Their runtime cache is capped at 80 entries for 30 days and is removed on a new build. Quota pressure can evict runtime caches; clearing browser storage removes all offline content. High-resolution textures retain the existing standard-quality fallback.
