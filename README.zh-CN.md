...
顶栏的**分享此刻所见**会截取当前画面，并生成一条指回它的链接。链接复现同一模拟时刻、播放状态、跟随天体与取景方式；其中不含观测坐标和显示偏好，因此不会覆盖对方自己的设置。若目标应用接受图片——手机系统分享面板，或任何平台上的**保存图片**——截图会随链接一起发出，并带有天体名称与 UTC 时刻。其余场合，链接的社交卡片回退为同一天体的渲染图。
