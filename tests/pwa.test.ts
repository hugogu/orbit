...
void test('PWA routes exclude APIs, analytics, mutations and RSC responses', () => {
  ...
  for (const [prefix, locale] of [
    ...
    // The body index and the sky-event guide are cached like a body profile.
    assert.ok(isProfilePath(`/${prefix}/bodies`));
  ...
  for (const path of ['/bodies', '/zh-CN/bodiesx', '/zh-CN/bodies/earth/x'])
    assert.equal(isProfilePath(path), false, path);
});
...
void test('production worker bundles offline dependencies and changes revision with either build target', async (t) => {
  ...
  for (const [path, content] of [
    ...
    ['assets/app.js.map', '{}'],
    ['textures/2k_earth_daymap.jpg', 'earth day map'],
    ['textures/2k_earth_nightmap.jpg', 'earth night map'],
    ['textures/2k_stars_milky_way.jpg', 'sky map'],
    ['textures/2k_sun.jpg', 'sun map'],
    ['textures/2k_mars.jpg', 'base map'],
    ['textures/2k_moon.jpg', 'moon map'],
    ['textures/2k_jupiter.jpg', 'jupiter map'],
